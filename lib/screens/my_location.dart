...
import 'package:tictapp_driver/chats/order_chat.dart';
import 'package:tictapp_driver/screens/dashboard.dart';
...
class _MyLocationState extends State<MyLocation>
  ...
  String vanIDSaved = "";
  int loadCount = 0;
  saveLAtLng(Position position) async {
    ...
    if (loadCount < 2) {
      _controller.animateCamera(
          CameraUpdate.newLatLngBounds(boundsFromLatLngList(latLngList), 10));
    }
    ...
    vanIDSaved = (preferences.getInt(id)).toString();
    Map<String, dynamic> dtat = {
      'location': {
        "lat": position.latitude,
        "lng": position.longitude,
        "vanId": (preferences.getInt(id)).toString(),
        "isMoving": isMoving,
        "heading": position.heading,
        "driverName": preferences.getString(firstname) +
            " " +
            preferences.getString(lastname),
        "address": currentLocationString,
        "speed": ((position.speed * 60) * 60) / 1000
      }
    };
    ...
    try {
      await databaseReference
          .collection("location")
          .document(widget.orderObject['order_id'])
          .collection("location")
          .getDocuments()
          .then((QuerySnapshot snapshot) {
        snapshot.documents.forEach((f) {
          if (f.data['location']['vanId'] ==
                  ...
                  FlatButton(
                      onPressed: () {
                        Map<String, dynamic> mapToSend = {
                          'requests': {
                            "lat": requestList[i]['lat'],
                            "lng": requestList[i]['lng'],
                            "vanId": vanIDSaved,
                            "isAccepted": true,
                            'address': requestList[i]['address'],
                            "customer_name": requestList[i]['customer_name'],
                            'customer_id': requestList[i]['customer_id'],
                            'phone': requestList[i]['phone'],
                          }
                        };
                        //requestList[i]['isAccepted'] = true;
                        databaseReference
                            .collection("user_requests")
                            .document(requestIdList[i])
                            .updateData(mapToSend);
                      },
                      child: Text(
                        "ACCEPT",
                        style: TextStyle(
